...
def train(net, args, train_loader, val_loaders, test_loaders, device, mode='synth'):
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        for idx, data in enumerate(train_loader):
            if args.get_theta:
                img, mask, axis, axis_gs, is_syn, a_lbl, r1 = data[0]
                img2, mask2, axis2, axis_gs2, is_syn2, a_lbl2, r2 = data[1]
            ...
            _mask2 = (mask2 > 0).float().to(device)
            axis_out, mask_out, total_loss, losses = net(img=[img.to(device), img2.to(device)], lbl=[axis_gs.float().to(device), axis_gs2.float().to(device)], mask=[_mask, _mask2], is_syn = is_syn.to(device), a_lbl=a_lbl, angle = r1)
            ...
            if (idx == len(train_loader) - 1) and not args.wandb_off:
                ...
                print("Train Loss", running_loss / running_sample)
        ...
        if epoch % 1 == 0 or (epoch==1):
            ...
            print("now f1:", _max_f1, "max f1:", max_f1)
    ...
    with torch.no_grad():
        for i, test_loader in enumerate(test_loaders):
            ...
            for idx, data in enumerate(test_loader):
                if args.get_theta:
                    img, mask, axis, axis_gs, is_syn, a_lbl, angle = data
                    a_lbl = a_lbl.to(device)
                else:
                    img, mask, axis, axis_gs, is_syn, _ = data
                    a_lbl = None
                ...
                axis_out, mask_out, total_loss, losses = net(img=[img.to(device), img.to(device)], lbl=[axis_gs.float().to(device), axis_gs.float().to(device)], mask=[_mask, _mask], is_syn = is_syn.to(device), a_lbl=a_lbl, angle=angle)
            ...
            prec, rec, f1 = axis_eval.f1_score()
            recs.append(rec), precs.append(prec), f1s.append(f1), f1_maxs.append(f1.max())
            metrics = axis_eval.calculate_metrics()
            max_f1_index = torch.argmax(metrics["f1"]).item()
            best_metrics = {
                "threshold_index": max_f1_index,
                "f1": metrics["f1"][max_f1_index].item(),
                "recall": metrics["recall"][max_f1_index].item(),
                "precision": metrics["precision"][max_f1_index].item(),
                "tp": metrics["tp"][max_f1_index].item(),
                "fp": metrics["fp"][max_f1_index].item(),
                "fn": metrics["fn"][max_f1_index].item(),
                "tn": metrics["tn"][max_f1_index].item(),
                "oe": metrics["oe"][max_f1_index].item(),
                "pcc": metrics["pcc"][max_f1_index].item(),
                "kappa": metrics["kappa"][max_f1_index].item()
            }
            print("\n=== best metrics ===")
            for name, value in best_metrics.items():
                if name == "threshold_index":
                    print(f"阈值索引: {value}")
                else:
...
if __name__ == '__main__':
    ...
    train_loader = data.DataLoader(trainset, batch_size=args.bs_train, shuffle=True, num_workers=0, drop_last=True)
    val_loader = data.DataLoader(valset, batch_size=1, shuffle=False, num_workers=0)
    test_loader = data.DataLoader(testset, batch_size=1, shuffle=False, num_workers=0)

    # use LDRS，NYU，SDRW dataset
    # from dendi_loader import SymmetryDatasets
    # testset = SymmetryDatasets(dataset=['SYM_NYU'], split='test', input_size=[args.input_size, args.input_size])
