    ...
    with torch.no_grad():
        for i, test_loader in enumerate(test_loaders):
            for idx, data in enumerate(test_loader): 
                img, im_path = data 
                axis_out = net(img=[img.to(device), img.to(device)], lbl=[img.float().to(device), img.float().to(device)], mask=None, is_syn=False, sym_type=sym_type, vis_only=True)
                ...
                axis_path = './demo/result/%s_%s_axis.png' % (im_path[0].split('/')[-1][:-4], sym_type,)
                cv2.imwrite(axis_path, np.uint8(255 * axis))

                img_path = './demo/result/%s_%s.png' % (im_path[0].split('/')[-1][:-4], sym_type, )
                plt.imsave(img_path, np.clip(attended_img, 0, 1))

                heatmap_path = './demo/result/%s_%s_heatmap.png' % (im_path[0].split('/')[-1][:-4], sym_type,)
